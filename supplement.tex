...
\begin{proof}
	...
	The critical point $\bar{\beta}$ satisfies the following equation:
	$$
	b\sum_{i=1}^{k-1} u_i \exp(u_i \beta) = a u \exp(-u\beta)
	$$
	Replacing $a\exp(-u\beta)$ by the above expression in \eqref{eq:gvbeta} we get
	$$
	g_v(\bar{\beta}) = b\sum_{i=1}^{k-1}
	$$
\end{proof}
\begin{remark}
