...
\begin{lemma}
    ...
    $P_{SSBM}(G | \sigma^{(j)},j\in[m]) = P_{SSBM}(\pi(G) | \sigma^{(j)},j\in[m])$
\end{lemma}
\begin{proof}
	...
	we conclude that $P_{SSBM}(G) = P_{SSBM}(\pi(G))$. Using Lemma \ref{lem:post_independent}, it is not difficult to show that
	$P_{SSBM}(G | \sigma^{(j)},j\in[m]) = P_{SSBM}(\pi(G) | \sigma^{(j)},j\in[m])$
\end{proof}
\end{document}
