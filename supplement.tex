...
\end{lemma}
\begin{proof}
	The idea is similar with Lemma \ref{lem:dist_sigma}. Firstly
	we establish $\frac{P_{\sigma | G}(T_+ = 1)}{P_{\sigma | G}(T_+ = 0)} \geq C\sum_{i=1, X_i=1}^n \exp(u(B_i - A_i))$
	...
	\end{equation}
	First, we write $
	\frac{P_{\sigma | G}(T_+ = 1)}{P_{\sigma | G}(T_+ = 0)} =
	...
	$$
	Among all $v'$, we consider $v'$ is all one vector ($\sigma_i^{(r)} = X_i = 1$ for all $r=1, \dots, )$
\end{proof}
