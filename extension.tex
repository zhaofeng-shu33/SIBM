...
If $\beta^\ast/\beta$ is not an integer and $m < \tilde{m} + \frac{1}{k-1}$, then the success probability of all recovery algorithms approaches $0$ as $n\to\infty$. If $\beta^\ast/\beta$ is an integer and $m < \tilde{m} -1$, then the success probability of all recovery algorithms approaches $0$ as $n\to\infty$.
...
The proof of the converse results, i.e., even ML algorithm cannot recover $X$ with probability $1-o(1)$ when $\lfloor \frac{(k-1)(m+1)}{k} \rfloor \beta < \beta^\ast$, also relies on the structural result and it is rather similar to the proof of $\beta\le\beta^\ast$ for the single-sample case. We refer the readers to Section~\ref{sect:converse} for details.
...
\begin{proposition}  \label{prop:zj}
	Let $a,b,\alpha> 0$ be constants satisfying that $\sqrt{a}-\sqrt{b} > \sqrt{2}$ and $\alpha>b\beta^\ast$. Let 
	$
...
\begin{lemma}  \label{lm:cvs}
	...
	\end{equation}
	If $(m-u_r)\beta<\beta^\ast$, then $P_{\SIBM}\big(T_r = \Theta(n^{g((m-u_r)\beta)}) \big) = 1-o(1)$.
\end{lemma}
\begin{proof}
	...
	\begin{align*}
	& \underline{C}
	\sum_{r=1}^{k-1}\exp\big(k(m-u_0)\beta (A^r_i-A^0_i) \big) \\
	\le
	...
	\begin{enumerate}
		...
		\sum_{r=1}^{k-1}\exp\big(k(m-u_0)\beta (A^r_i-A^0_i) \big)$ and $P(\overline{S}_i=1)=\overline{C}
		\sum_{r=1}^{k-1}\exp\big(k(m-u_0)\beta (A^r_i-A^0_i) \big)$ for all $i\in[n]$.
	\end{enumerate}
	...
	\sum_{i\in[n]_+} \sum_{r=1}^{k-1}\exp\big(k(m-u_0)\beta (A^r_i-A^0_i) \big) , \quad
	\Var \Big(\sum_{i\in[n]_+}\underline{S}_i \Big) \le \underline{C}
	\sum_{i\in[n]_+} \sum_{r=1}^{k-1}\exp\big(k(m-u_0)\beta (A^r_i-A^0_i) \big), \\
	...
	\end{align*}
	where we use the fact that the variance of a Bernoulli random variable is always upper bounded by its expectation.
	In Proposition~\ref{prop:con}, we have shown that for $(m-u_0)\beta\le \beta^\ast$, there is a set $\cG_{\con}$ such that (i) $P(G \in \cG_{\con}) = 1-o(1)$ and (ii) for every $G\in \cG_{\con}$, 
	$
	\sum_{i=1}^n \exp\big(k(m-u_0)\beta (A^r_i-A^0_i) \big)
	=(1+o(1)) n^{g((m-u_0)\beta)} .
	$
    Since the size of $[n]_0$ is $1/k$ of $[n]$, for every $G\in \cG_{\con}$, 
	$
	\sum_{i\in[n]_0} \exp\big(k(m-u_0)\beta (A^r_i-A^0_i) \big)
	=\Theta (n^{g((m-u_0)\beta)}) .
	$
	Therefore, for every $G\in\cG_{\good}\cap\cG_{\con}$, 
	\begin{align*}
	& E \Big[ \sum_{i\in[n]_+}\underline{S}_i \Big] = \Theta(n^{g((m-u_0)\beta)}) , \quad
	\Var \Big( \sum_{i\in[n]_+}\underline{S}_i \Big) = O(n^{g((m-u_0)\beta)}) , \\
	& E \Big[ \sum_{i\in[n]_+}\overline{S}_i \Big] = \Theta(n^{g((m-u_0)\beta)}) , \quad
	\Var \Big( \sum_{i\in[n]_+}\overline{S}_i \Big) = O(n^{g((m-u_0)\beta)}) 
	\end{align*}
	conditioning on the event $\{\sigma^{(j)}\in  \Lambda(G, z)
	\text{~and~} \dist(\sigma^{(j)}, X) \le n^\theta
	\text{~for all~} j\in[m]\}$. Since $g((m-u_0)\beta)> 0$ for all $(m-u_0)\beta< \beta^\ast$, by Chebyshev's inequality we know that both $\sum_{i\in[n]_+}\underline{S}_i=\Theta(n^{g((m-u_0)\beta)})$ and $\sum_{i\in[n]_+}\overline{S}_i=\Theta(n^{g((m-u_0)\beta)})$ with probability $1-o(1)$ conditioning on the event $\{\sigma^{(j)}\in  \Lambda(G, z)
	\text{~and~} \dist(\sigma^{(j)}, X) \le n^\theta
	\text{~for all~} j\in[m]\}$.
	Since $\underline{S}_i\le \phi_i\le \overline{S}_i$ for all $i\in[n]$, we also have $T_0=\Theta(n^{g((m-u_0)\beta)})$ with probability $1-o(1)$ conditioning on the event $\{\sigma^{(j)}\in  \Lambda(G, z)
	\text{~and~} \dist(\sigma^{(j)}, X) \le n^\theta
	\text{~for all~} j\in[m]\}$. Combining this with \eqref{eq:ljrc}, we obtain that for every $G\in\cG_{\good}\cap\cG_{\con}$,
	$T_0=\Theta(n^{g((m-u_0)\beta)})$ with probability $1-o(1)$ conditioning on $\dist(\sigma^{(j)}, X) \le n/k$ for all $j\in[m]$. Finally, the lemma follows from $P(G\in\cG_{\good}\cap\cG_{\con})=1-o(1)$.
\end{proof}
\begin{lemma}\label{lem:mk}
	...
	(k-1) \lfloor{m + 1 \over k} \rfloor& \leq  \lfloor {k-1 \over k}(m+1) \rfloor \label{eq:m2}
	\end{align}
\end{lemma}
\begin{proof}
...
\begin{proposition}
	...
	(X,G,\{\sigma^{(1)},\dots,\sigma^{(m)}\})\sim \SIBM(n,k,a\log(n)/n, b\log(n)/n,\alpha,\beta, m) .
	$$
	If $\lfloor (k-1)\frac{m+1}{k} \rfloor \beta<\beta^\ast$, then no algorithm can recover $X$ from the samples with constant success probability, i.e., the success probability of any algorithm is $o(1)$.
\end{proposition}
\begin{proof}
	From Lemma \ref{lem:mk} we have $m-\lfloor \frac{m+1}{k}  \rfloor \le \lfloor (k-1)\frac{m+1}{k} \rfloor$. If $\lfloor (k-1)\frac{m+1}{k} \rfloor \beta<\beta^\ast$, then $(m-\lfloor \frac{m+1}{k}  \rfloor) \beta<\beta^\ast$.
	Now pick a vector $v\in W^m$ such that it has $\lfloor \frac{m+1}{k} \rfloor$ coordinates being $\omega^r$ for $r=0, \dots, k-2$ and $(m-(k-1)\lfloor \frac{m+1}{k} \rfloor)$ coordinates being $\omega^{k-1}$.
	Since $(k-1)\lfloor \frac{m+1}{k} \rfloor \leq \lfloor (k-1)\frac{m+1}{k} \rfloor$ by Lemma~\ref{lem:mk},
...
\begin{lemma}
	For given $X, W \in W^n$, let $f^*=\arg\min_{f \in \Gamma } d(f(Y), X)$, then $d(X,f^*(Y)) \leq \frac{k-1}{k}n$.
\end{lemma}
\begin{lemma}\label{lem:thm23eq}
\begin{align}
\lfloor \frac{m+1}{2} \rfloor \beta>\beta^\ast \iff  & m\ge 2 \Big\lfloor \frac{\beta^\ast}{\beta} \Big\rfloor +1 \label{eq:m2} \\
\lfloor \frac{(k-1)(m+1)}{k} \rfloor \beta <\beta^\ast  \iff & m < \tilde{m} + \frac{1}{k-1} \textrm{ if } \beta^*/\beta \textrm{ is not an integer } \notag \\
& m < \tilde{m} -1 \textrm{ if } \beta^*/\beta \textrm{ is an integer } \label{eq:m22}
\end{align}
So Theorem~\ref{thm:wt1} and Theorem~\ref{thm:wt2} give the same threshold.
\end{lemma}
\begin{proof}
	First, we give a proof of Equation \eqref{eq:m2}: $\lfloor \frac{m+1}{2} \rfloor \beta>\beta^\ast$ implies that $\frac{\beta^\ast}{\beta}<\lfloor \frac{m+1}{2} \rfloor$.
	The smallest integer that is larger than $\frac{\beta^\ast}{\beta}$ is $\lfloor \frac{\beta^\ast}{\beta}\rfloor +1$, so $\lfloor \frac{\beta^\ast}{\beta} \rfloor +1 \le \lfloor \frac{m+1}{2} \rfloor\le \frac{m+1}{2}$, and thus $m\ge 2 \Big\lfloor \frac{\beta^\ast}{\beta} \Big\rfloor +1$. Now assume $m\ge 2 \Big\lfloor \frac{\beta^\ast}{\beta} \Big\rfloor +1$, then $\frac{m-1}{2} \ge \lfloor \frac{\beta^\ast}{\beta} \rfloor$. Since the right hand side is an integer, we have $\lfloor \frac{m+1}{2} \rfloor = \lfloor \frac{m-1}{2} \rfloor +1  \ge \lfloor \frac{\beta^\ast}{\beta} \rfloor +1 >\frac{\beta^\ast}{\beta}$.
