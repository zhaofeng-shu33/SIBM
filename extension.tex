...
\noindent (i)
$P(G\in\cG^{(r)}) \ge 1- k n^{r\tilde{g}(\beta)/8}$ ,
...
$$
\tilde{D}(G):=|\{i\in[n]:A^1_i- A^0_i\ge 0\}|
\text{~~and~~}
\tilde{D}_i(G):=\mathbbm{1}[A^1_i- A^0_i\ge 0] ,
$$
...
=\sum_{i=1}^n P( A^1_i- A^0_i\ge 0).
$$
Taking $t=0$ into \eqref{eq:upba}, we have
$P(B_i- A_i\ge 0)\le \exp\big(\log(n)(-\frac{(\sqrt{a}-\sqrt{b})^2}{k} +o(1)) \big)$. Therefore,
$$
E[\tilde{D}(G)]\le n \exp\Big(\log(n)\big(-\frac{(\sqrt{a}-\sqrt{b})^2}{k} +o(1) \big) \Big)
