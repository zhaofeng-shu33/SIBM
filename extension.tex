...
\end{equation}
	...
	B_{\bar{\sigma}} & \sim Binom(\frac{n}{k}I + \frac{1}{2}\sum_{i=0}^{k-1}  (-2 I'_i I_i  + I'^2_i - \sum_{j=0}^{k-1} I^2_{ji}) , q)\\
	A_{\bar{\sigma}} & \sim Binom(\frac{n}{k}I - \frac{1}{2}\sum_{i=0}^{k-1}  (I^2_i + \sum_{j=0}^{k-1} I^2_{ij}), p) \label{eq:A_w}
	\end{align}
...
\begin{proof}
	...
	\begin{align*}
	|A_{\bar{\sigma}}| &= \frac{n}{k}I - \frac{1}{2}\sum_{i=0}^{k-1}  (I^2_i + \sum_{j=0}^{k-1} I^2_{ij}) \\
...
\begin{lemma}\label{lem:enhanced_fb}
	For $t\in [\frac{1}{k}(b-a), 0]$
	and $ I \le n/\log^{\delta}(n)$
	\begin{equation} \label{eq:upmpt}
	\begin{aligned}
	& P(B_{\bar{\sigma}}-A_{\bar{\sigma}}\ge t I \log(n))  \\
	\le & \exp\Big(I\log(n)
...
\begin{proof}
	...
	$B_{\bar{\sigma}}\sim\Binom(|B_{\bar{\sigma}}|,\frac{b\log(n)}{n})$, and they are independent. For $s>0$, the moment generating function of $B_{\bar{\sigma}}-A_{\bar{\sigma}}$ for $ I \le n/\log^{\delta}(n)$ can be bounded from above as follows:
	\begin{align*}
	...
	\end{align*}
	Using the conclusion that $|B_{\bar{\sigma}}|=\frac{n}{k}I + \frac{1}{2}\sum_{i=0}^{k-1}  (-2 I'_i I_i  + I'^2_i - \sum_{j=0}^{k-1} I^2_{ji})$ and
	$|A_{\bar{\sigma}}| = \frac{n}{k}I - \frac{1}{2}\sum_{i=0}^{k-1}  (I^2_i + \sum_{j=0}^{k-1} I^2_{ij})$.
	By estimating the order of residual term, $|A_{\bar{\sigma}}| = \frac{n}{k}I + O(I^2)$,
	 $|B_{\bar{\sigma}}| = \frac{n}{k}I + O(I^2)$.
	 
	\begin{align*}
	E[e^{s(B_{\bar{\sigma}}-A_{\bar{\sigma}})}] & \le
	\exp(\frac{\log n}{n}((\frac{n}{k}I + O(I^2))(be^s - b) +(\frac{n}{k} I + O(I^2))(ae^{-s}-a) + \frac{n}{k}IO(\frac{\log n}{n}) ))
	 \\
	& =
	\exp\Big(\frac{I \log(n)}{k}(a e^{-s}+b e^s-a-b +
	O(\log^{-\delta}(n))) \Big),
	\end{align*}
	The last equality follows from the assumption that $ I \le n/\log^{\delta}(n)$.
	By Chernoff bound, for $s>0$, we have
	\begin{align*} 
	& P(B_{\bar{\sigma}}-A_{\bar{\sigma}}\ge t I \log(n))\le
	\frac{E[e^{s(B_{\bar{\sigma}}-A_{\bar{\sigma}})}]}{e^{st I \log(n)}}  \\
	\le & \exp\Big(\frac{I\log(n)}{k} \big(a e^{-s}+b e^s -kst -a-b
...
\Pr(  B_{\bar{\sigma}} - A_{\bar{\sigma}}    < 0 , \forall  \bar{\sigma} \in \Xi(v) ) \\
& \leq I_1
+ \Pr(D(v,s) | B_{\bar{\sigma}} - A_{\bar{\sigma}}    < 0, \forall \bar{\sigma} \in \Xi(v))
\end{align*}
...
& = \sum_{t\log n = -A_{\bar{\sigma}} }^{-1} \sum_{\bar{\sigma} \in \Xi(v)} \mathbbm{1}[B_{\bar{\sigma}} - A_{\bar{\sigma}}  = t \log n] \exp ( \beta  t\log n)\leq \\ 
&
\sum_{t\log n =\tau}^{-1} \sum_{\bar{\sigma} \in \Xi(v)}  \mathbbm{1}[B_{\bar{\sigma}} - A_{\bar{\sigma}}  = t \log n]
\exp ( \beta  t\log n) + n^{r+r\beta(b-a)/k}
\end{align*}
...
by Lemma xxx, $P(D_{v, s}) \leq 2 n^{r\tilde{g}(\beta)/4}$ where
$s=O(n^{r\tilde{g}(\beta)/2})$. Therefore, by union bound
\begin{align*}
