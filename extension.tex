...
By Lemma~\ref{lm:et},
$$
P_{\sigma|G}(\sigma_i^{(j)} \neq X_i \big| \sigma^{(j)}\in \Lambda(G,z) ,\dist(\sigma^{(j)}, X) \le n^{\theta} ) 
...
C \sum_{r=1}^{k-1}\sum_{i=1}^n \exp\Big(ku \big(\beta+\frac{\alpha\log(n)}{n} \big) (A^r_i-A^0_i ) \Big) + o(1) ,
$$
