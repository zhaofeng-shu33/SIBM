...
\end{theorem}
\begin{proof}
	This theorem is a special case of Lemma \ref{lm:cvs} when $m=1$. Actually, Lemma \ref{lm:cvs}
	shows that 
...
$$
P_{\sigma|G} \big(D_r\big) \le 
C\sum_{i=1}^n \exp\Big(u \big(\beta+\frac{\alpha\log(n)}{n} \big) (A^r_i-A^0_i ) \Big) = (1+o(1))n^{g(u\beta)}.
$$
Further
\begin{align*}
...
\begin{proof}
	...
	\begin{align*}
	& \underline{C}
	\exp\big((m-u_0) \beta (A^r_i-A^0_i) \big) \\
	\le
	& P_{\sigma|G} \big( (\sigma_i^{(1)},\dots,\sigma_i^{(m)})=v ~\big|~ \sigma^{(j)}\in  \Lambda(G, z)
	\text{~and~} \dist(\sigma^{(j)}, X) \le n^\theta
	\text{~for all~} j\in[m] \big) \\
	\le & \overline{C}
	\sum_{r=1}^{k-1}\exp\big((m-u_0) \beta (A^r_i-A^0_i) \big)
	\end{align*}
	...
	\begin{enumerate}
		...
		\sum_{r=1}^{k-1}\exp\big(u_r \beta (A^r_i-A^0_i) \big)$ and $P(\overline{S}_i=1)=\overline{C}
		\sum_{r=1}^{k-1}\exp\big(u_r \beta (A^r_i-A^0_i) \big)$ for all $i\in[n]$.
	\end{enumerate}
	...
	\sum_{i\in[n]_+} \sum_{r=1}^{k-1}\exp\big(u_r \beta (A^r_i-A^0_i) \big) , \quad
	\Var \Big(\sum_{i\in[n]_+}\underline{S}_i \Big) \le \underline{C}
	\sum_{i\in[n]_+} \sum_{r=1}^{k-1}\exp\big(u_r \beta (A^r_i-A^0_i) \big), \\
...
\end{proposition}
\begin{proof}
	From Lemma \ref{lem:mk}, we choose $v \in W^m$ such that it has $\lfloor \frac{m+1}{2} \rfloor$ coordinates being $1$
