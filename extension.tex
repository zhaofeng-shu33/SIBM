...
\begin{proof}
	...
	By \eqref{eq:single_sigma}, we know that ,
	$P_{\sigma|G}(\sigma_i^{(j)} = \omega^s \big| \sigma^{(j)}\in \Lambda(G,z) ,\dist(\sigma^{(j)}, X) \le n^{\theta} )$ differ from
	$\exp(\beta (A^s_i-A^0_i) )$
	by at most a constant factor.
	Moreover, since the $m$ samples are independent given the graph $G$, there are $u_0$ samples which takes value $\sigma_i^{(j)} = 1$. Since the probability
