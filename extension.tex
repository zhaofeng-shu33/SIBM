...
< (k-1)^rn^{-r(\delta-o(1))} .
$$
This immediately implies that  $P_{\SIBM} (\dist(\sigma, \Gamma)<n^{\theta} ) = 1- o(1)$ for any $\delta>0$.
...
\begin{lemma}\label{lem:small}
	$\alpha< b \beta$. When $\dist(\bar{\sigma}, \mathbf{1}_n) \geq \frac{n}{\log^{\delta} n}$ where $0<\delta < 1$ and $\arg\,\min_{\sigma'\in \Lambda} \dist(\bar{\sigma}, \sigma') = \mathbf{1}_n$. Then
	$P_{\sigma | G}(\sigma = \bar{\sigma} ) > \exp(-Cn) P_{\sigma | G}(\sigma = \mathbf{1}_n)$
...
\end{theorem}
\begin{proof}
	Let $D(X):=X = \arg\dist_{X'\in S(X)}(\sigma, S(X'))$,
	we prove the following equivalent form:
	$$
	P_{\SIBM}(\dist(\sigma, X) = \Theta(n^{g(\beta)}) ~\big|~ D(X) ) = 1-o(1) .
	$$
	...
	\text{~and~} \dist(\sigma, X) \le n^\theta ~\big|~ D(X) \big) 
	=1- O(n^{-4}) ,
	\end{equation}
...
\begin{lemma}  \label{lm:cvs}
	...
	Without loss of generality we assume that the aligned samples satisfy  $\dist(\sigma^{(j)}, X) \le n^{\theta}$ for all $j\in[m]$.
	Define 
...
\begin{proof}
	...
	=\Theta (n^{g(u_r \beta)}) .
	$
	Therefore, for every $G\in\cG'\cap\cG_{\con}$, 
	\begin{align*}
	...
	\text{~and~} \dist(\sigma^{(j)}, X) \le n^\theta
	\text{~for all~} j\in[m]\}$. Combining this with \eqref{eq:ljrc}, we obtain that for every $G\in\cG'\cap\cG_{\con}$,
	$T_0=\Theta( n^{g(u_r\beta)})$ with probability $1-o(1)$ conditioning on $\dist(\sigma^{(j)}, X) \le n/k$ for all $j\in[m]$. Finally, the lemma follows from $P(G\in\cG'\cap\cG_{\con})=1-o(1)$.
\end{proof}
