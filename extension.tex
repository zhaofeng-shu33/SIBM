...
We prove that the number of samples needed for exact recovery of $X$ is at least $\Omega(\log^{\delta}(n))$.
...
\begin{proof}
	...
	\begin{align*}
	&\Pr([B_w - A_w] >  \frac{\alpha \log n}{\beta n} N_w  - \frac{C}{\beta}n) \leq (E[exp(sZ_{ij})])^{|B_w|}(E[\exp(-sZ_{ij})])^{|A_w|} \exp(-s(\frac{\alpha \log n}{\beta n} N_w  - \frac{C}{\beta}n)) \\
	...
	$$
	\frac{\log n}{n} [|A_w|(b(e^s -1)+ a(e^{-s} - 1)) + N_w (b(e^s - 1)-s\frac{\alpha}{\beta})]  + s \frac{C}{\beta}n
	$$
