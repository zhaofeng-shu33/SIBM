...
{\renewcommand{\thefootnote}{}\footnotetext{
		...
		Feng Zhao is with Department of Electronics, Tsinghua University, email: zhaof17@mails.tsinghua.edu.cn.
		Min Ye and Shao-Lun Huang are
		with the Data Science and Information Technology Research Center, Tsinghua-Berkeley Shenzhen Institute, Tsinghua Shenzhen International Graduate School, Shenzhen 518055, China.
}}
...
\begin{proof}
	...
	The second inequality is achieved if $n_0 = n - \frac{n}{\log^{\delta} n}$.
	When $n_0 < \frac{n}{2}$,
	$N_{\bar{\sigma}} \geq \frac{n^2 - nn_0}{2} \geq \frac{n^2}{4}$ and the second inequality is achieved when $n_0 = \frac{n}{2}$.
	So generally we have $\frac{n^2}{\log^{\delta} n}(1+o(1)) \leq N_{\bar{\sigma}} \leq \frac{n^2}{2}$.
	...
	$N_{\bar{\sigma}} \geq \frac{n^2}{\log^{\delta} n}$ we have
	\begin{align*}
...
\begin{proof}
	...
	\end{align*}
	Use the conclusion that $|B_{\bar{\sigma}}|=\frac{n}{k}|\cI| + \frac{1}{2}\sum_{i=0}^{k-1}  (-2 I'_i I_i  + I'^2_i - \sum_{j=0}^{k-1} I^2_{ji})$ and
	$|A_{\bar{\sigma}}| = \frac{n}{k}|\cI| - \frac{1}{2}\sum_{i=0}^{k-1}  (I^2_i + \sum_{j=0}^{k-1} I^2_{ij})$.
...
\begin{align*}
& P_{\sigma|G}(D_u) \\
\le &  \binom{m}{u}\sum_v \exp\Big(\big(\beta+\frac{\alpha\log n}{n} \big) (\sum_{r=1}^{k-1} u_r A^r_i- u A^0_i + 2uz) \Big)
...
$$
On the other hand,
if $t^\ast > 0$, or equivalently $\beta > \frac{1}{2}\log\frac{a}{b}$, then
$f_{\beta}(t)$ takes maximum value at $t=0$. Since $f_{\beta}(t)$ is concave, we have
$f_{\beta}(t) \leq f_{\beta}(0) + f_{\beta}'(0)t$.
Therefore, $\sum_{t\log n=-n/k}^{-1}
\exp( (f_{\beta}(t) -1)\log n ) \leq n^{f_{\beta}(0)-1} \sum_{i=0}^{\infty } q^i \leq \frac{1}{1-q} n^{f_{\beta}(0)-1} $ 
where $q=\exp(-f_{\beta}'(0)) < 1$.
As a consequence,
\begin{align*}
\mathbb{E} \Big[\exp\big(\beta (A^r_i-A^0_i) \big) ~\Big|~ G\in\cG_1 \Big] = (1+o(1))
