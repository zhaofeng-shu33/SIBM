...
If $b\beta >\alpha$, then for any (arbitrarily large) $r>0$, there exists $n_0(r)$ such that for all even integers $n>n_0(\delta, r)$,
$$
P_{\SIBM} \Big(\dist(\sigma^{(i)}, \Theta)< kn/\log^{1/3}(n)
\text{~~for all~} i\in[m] \Big) \ge 1- n^{-r} .
$$
\end{proposition}
\begin{proof}
...
$$
\sum_{r=1}^k Q_{rr} < \frac{k-1}{k}n^2 - \frac{2kn^2}{\log^{1/3} n}
+O(n^2/\log^{2/3}(n)).
$$
...
\le \exp\Big(-(a\beta-b\beta)kn\log^{2/3}(n)
+ O(n\sqrt{\log(n)}) \Big)
<k^{-n} e^{-n}
$$
