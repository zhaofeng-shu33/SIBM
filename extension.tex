...
\end{theorem}
\begin{proof}
	This theorem is a special case of Lemma \ref{lm:cvs} when $m=1$.
	For $s\neq r$, we have
	$$
	P_{\SIBM}(|\{j\in [n] | \sigma_j = \omega^s, X_j = \omega^r \}| = \Theta(n^{g(\beta)}) ~\big|~ \dist(\sigma, X) \le n^{\theta}) = 1-o(1) .
	$$
	From $\dist(\sigma, X) =\sum_{r\neq s} |\{j\in [n] | \sigma_j = \omega^s, X_j = \omega^r \}|$, we have:
	$$
	P_{\SIBM}(\dist(\sigma, X) = \Theta(n^{g(\beta)}) ~\big|~ \dist(\sigma, X) \le n^{\theta}) = 1-o(1) .
	$$
