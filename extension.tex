...
$$
Let $\bar{W} = \{w, \dots, w^{k-1}\}$, and we consider the event
$$
D(v, s): \sum_{\bar{\sigma}_{i_j} = v_j \cdot X_{i_j}, j=1,\dots, r} \exp((\beta + \frac{\alpha \log n}{n}) [B_{\bar{\sigma}} - A_{\bar{\sigma}}]) > s
$$
where $v \in \bar{W}^r$ and the number of summed term in $D(v, s)$ is $\binom{n}{r}$.

Following the main idea in Section \ref{sect:why}, we consider an enhanced conclusion of Lemma \ref{lem:fb}.
\begin{lemma}
	For $t\in [\frac{1}{k}(b-a), 0]$
	and $ I \le n/\log^{\delta}(n),v$
