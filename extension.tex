...
By \eqref{eq:I_1}, we have $P(G\in\cG_1)=1-o(1)$. We will prove that
\begin{align}
...
= & E[\exp\big(\beta (A'^r_i-A^0_i) \big)] E[\exp\big(\beta (A'^r_j-A^0_j) \big)]  \Big( E[\exp(2k\beta \xi_{ij})] -
...
$$
E \Big[ \sum_{i=1}^n  \exp\big(\beta (A^r_i-A^0_i) \big) ~\Big|~ G\in\cG_1 \Big] 
= (1+o(1)) E \Big[ \sum_{i=1}^n  \exp\big(\beta (A^r_i-A^0_i) \big) \Big]
= (1+o(1)) n^{g(\beta)}  .
$$
...
D_r(G,t) \exp\big(\beta t \log(n) \big) .
\end{equation}
...
E \Big[ \sum_{i=1}^n  \exp\big(\beta (A^r_i-A^0_i) \big) \Big]     
& =  \sum_{t\log(n)=-n/k}^{n/k}
E \big[ D(G,t) \exp\big(\beta t \log(n) \big) \big]  \\
...
\begin{align*}
& \sum_{t\log(n)=-n/k}^{n/k}
E\big[ D_r(G,t) \exp\big(\beta t \log(n) \big) \big]  \\
...
$$
\sum_{t\log(n)=-n/k}^{n/k}
E\big[ D_r(G,t) \exp\big(\beta t \log(n) \big) \big] = (1+o(1))
\sum_{t\log(n)=-n/k}^{-1}
E\big[ D_r(G,t) \exp\big(\beta t \log(n) \big) \big] ,
$$
i.e.,
...
Then we have
\begin{align*}
E \big[  \exp\big(\beta (A^r_i - A^0_i + A^r_j - A^0_j ) \big) \big] & = E[\exp(2k\beta \xi_{ij})] E[\exp(\beta (A'^r_i - A^r_i)]
