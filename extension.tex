...
\begin{proof}
	...
	\end{equation}
	Corollary~\ref{cr:1} together with Proposition~\ref{prop:43}, \ref{prop:con} implies that there is an integer $z>0$ and a set $\cG'$ such that
	\begin{enumerate}[label=\arabic*)]
		\item $P(G\in\cG') \ge 1- O(n^{-4})$
	...
	From property 3), $G\in\cG'$, $\sum_{i=1}^n \exp\big(\beta (A^r_i-A^0_i) \big) = (1+o(1))n^{g(\beta)}$,
	we have 
	\begin{align*}
	...
	From \eqref{eq:qke} and Lemma 5 of \cite{ye2020exact}, we also have $\dist(\sigma,X)=\Theta(n^{g(\beta)})$ with probability $1-o(1)$ conditioning on
	$D(\phi)$.
	Combining this with \eqref{eq:hs}, we obtain that for every $G\in\cG'\cap\cG_1$, $P_{\sigma | G}(\dist(\sigma,X)=\Theta(n^{g(\beta)}) | D(\sigma, X)) = 1-o(1)$. Finally, \eqref{eq:conditionDist} follows from $P_G(\cG'\cap\cG_1)=1-o(1)$.
