...
= & \Theta\Big( \frac{\log(n)}{n} \Big) E[\exp\big(\beta (A^r_i-A^0_i) \big)] E[\exp\big(\beta (A^r_j-A^0_j) \big)]  ,
\end{align*} 
where the last equality holds because $\exp\big(\beta (A'^r_i-A^0_i) \big)$ differs from $\exp\big(\beta (A^r_i-A^0_i) \big)$  by a factor of at most $e^{k\beta}$. 
By \eqref{eq:pl} in Appendix~\ref{ap:um}, $E \big[  \exp\big(\beta (A^r_i-A^0_i) \big) ~\big|~ G\in\cG_1 \big] 
= (1+o(1)) E \big[  \exp\big(\beta (A^r_i-A^0_i) \big) \big]$ when $0<\beta\le\beta^\ast$. By Lemma \ref{lem:BijG} in Appendix~\ref{ap:um} we have $E[\exp\big(\beta (A^r_i-A^0_i+A^r_j-A^0_j) ~\big|~ G\in\cG_1 \big)] = (1+o(1)) E[\exp\big(\beta (A^r_i-A^0_i+A^r_j-A^0_j) \big)]$ when $0<\beta\le\beta^\ast$.
Therefore,
\begin{align*}
...
= (1+o(1)) n^{g(\beta)}  
$$
for all $\beta$. This gives us the same estimate as the conditional expectation in \eqref{eq:cg}. However, the unconditional variance can be much larger than the conditional one in \eqref{eq:sw}. Recall from \eqref{eq:mh1} that we use $\sum_{i=1}^n E[ \exp\big(2\beta (A^r_i-A^0_i) \big)]$ to bound the variance. 
...
\begin{align*}
P_{\SIBM}( \exists i \in [n], \Phi^r_i \ge u \big) \leq &P_{\SIBM}( \Phi^r_i \ge u , G \in \cG_{1}) + P(G \not\in \cG_{1}) \\
\leq & E_G[P_{\sigma |G}(D_r)]+ O(n^{-C})
\end{align*}
where the event $D_r: \exists i \in [n], \Phi^r_i \ge u, \sigma^{(j)}\in \Lambda(G,z) ,\dist(\sigma^{(j)}, X) \le n^{\theta} \text{~for all~} j\in[m]$.
To bound  $_G[P_{\sigma |G}(D_r)]$,
since
$$
...
$$
P_{\sigma|G} \big(D_r\big) \le 
C  \sum_{i=1}^n \exp\Big(u \big(\beta+\frac{\alpha\log(n)}{n} \big) (A^r_i-A^0_i ) \Big),
$$
where $C:=\binom{m}{u}\exp(2u\beta z)$ is a constant.
By Proposition \ref{prop:con}, if $u\beta>\beta^*$, there exists $\cG_2$ such that $P(G\not\in\cG_2) \leq n^{1-(\sqrt{a}-\sqrt{b})^2/k}$.
Then for every $G\in\cG_1 \cap\cG_{2}$,
$$
P_{\sigma|G} \big(D_r\big) \le 
C\sum_{i=1}^n \exp\Big(u \big(\beta+\frac{\alpha\log(n)}{n} \big) (A^r_i-A^0_i ) \Big) = (1+o(1))n^{g(\beta)}.
$$
Further
\begin{align*}
P_{\SIBM}( \exists i \in [n], \Phi^r_i \ge u \big)
\leq & E_G[P_{\sigma |G}(D_r)] + O(n^{-4}) \\
\leq &  (1+o(1))n^{g(\beta)} + P(G \not\in \cG_2) + O(n^{-C}) = o(1)
\end{align*}
We conclude that $P_{\SIBM} \big(\Phi^r_i \le u-1 \text{~for all~} i\in[n] \big)=1-o(1)$.
