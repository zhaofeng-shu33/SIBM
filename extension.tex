...
\le \sum_{t\log(n)=\lceil\frac{b-a}{k}\log(n) \rceil}^{-1}
 n^{f_{\beta}(t) + \frac{3}{2}\epsilon}
\right\} .
$$
Then by Markov inequality,
\begin{equation} \label{eq:mk}
P(G\in \cG^r(\epsilon))\ge 1-n^{-(\frac{3}{2}\epsilon-o(1))} >
1-n^{-\epsilon}
\end{equation}
...
{P_{\sigma|G}(\sigma=X)} 
& \le n(k-1)
\exp\big(\beta(b-a)\log(n) \big)  + (k-1)\sum_{t\log(n)=\lceil\frac{b-a}{k}\log(n) \rceil}^{-1}  n^{f_{\beta}(t) + \frac{3}{2}\epsilon} \\
& = (k-1)n^{f_{\beta}((b-a)/k)} + (k-1)\sum_{t\log(n)=\lceil\frac{b-a}{k}\log(n) \rceil}^{-1}  n^{f_{\beta}(t) + \frac{3}{2} \epsilon} ,
\end{aligned}
\end{equation}
...
+ (k-1)\sum_{t\log(n)=\lceil\frac{b-a}{k}\log(n) \rceil}^{-1}  n^{\tilde{g}(\beta)  + \frac{3}{2}\epsilon}  \\
& \le (k-1)n^{\tilde{g}(\beta)  + \frac{3}{2}\epsilon}
...
$$
P(G\in\cG^{(1)})\ge 1-(k-1)n^{\tilde{g}(\beta)/4}- (k-1)n^{1-\frac{(\sqrt{a}-\sqrt{b})^2}{k}+o(1)}> 1- 2(k-1)n^{\tilde{g}(\beta)/4},
$$
where the last inequality follows from the fact that $1-\frac{(\sqrt{a}-\sqrt{b})^2}{k}\le
\tilde{g}(\beta)<\tilde{g}(\beta)/4< 0$.
