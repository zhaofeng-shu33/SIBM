...
Below we use the notation $P_{\SIBM}(A):=E_G[P_{\sigma|G}(A)]$ for an event $A$, where the expectation $E_G$ is taken with respect to the distribution given by SSBM. In other words, $P_{\sigma|G}$ is the conditional distribution of  $\sigma$ given a fixed $G$ while $P_{\SIBM}$ is the marginal distribution of $\sigma$.
...
\begin{proof}
	...
	\begin{enumerate}[label=\arabic*)]
		...
		\sum_{r=1}^{k-1}\exp\big(\beta (A^r_i-A^0_i) \big)$ for all $i\in[n]$. The constants $\bar{C}$ and $\underline{C}$ are defined in \eqref{eq:qke}.
	\end{enumerate}
	...
	From \eqref{eq:qke} and Lemma 5 of \cite{ye2020exact}, we also have $\dist(\sigma,X)=\Theta(n^{g(\beta)})$ with probability $1-o(1)$ conditioning on the event $\{\sigma\in \Lambda(G,z) ,\dist(\sigma, X) \le n^{\theta}\}$. Combining this with \eqref{eq:hs}, we obtain that for every $G\in\cG'\cap\cG_1$, $P_{\sigma | G}(\dist(\sigma,X)=\Theta(n^{g(\beta)}) | D(\sigma, X)) = 1-o(1)$. Finally, \eqref{eq:conditionDist} follows from $P_G(\cG'\cap\cG_1)=1-o(1)$.
...
Then we have
\begin{align*}
\mathbb{E} \big[  \exp\big(\beta (A^r_i - A^0_i + A^r_j - A^0_j ) \big) \big] & = E[\exp(2\beta \xi_{ij})] E[\exp(\beta (A'^r_i - A^0_i)]
...
Therefore,
$$
\mathbb{E} \big[  \exp\big(\beta (A^r_i - A^0_i + A^r_j - A^0_j ) \big) \big] = (1+o(1))\sum_{t=-2\frac{n}{k}}^{-1} E[\exp(\beta t \log n)]
P(A^r_i - A^0_i + A^r_j - A^0_j = t\log n) 
$$
