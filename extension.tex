...
P(X=\bar{X}, \sigma_{i_1}^{(j)}=\sigma_{i_r}^{(j)}, j\in[m]) =
P(X=\hat{X}, \sigma_{i_1}^{(j)}=\sigma_{i_r}^{(j)}, j\in[m])
\end{equation}
We can also find a 1-1 mapping $\pi$ which satisfies $ \bar{X}_{i_r} = \hat{X}_{\pi(i_r)}$.
...
\begin{align*}
P(X=\bar{X}, \sigma_{i}^{(j)}=\sigma_{i'}^{(j)}, j\in[m], r\in[k])& = \sum_{G \in \cG_{[n]}} P(X=\bar{X}) P_{\SSBM}(G  | X = \bar{X})\prod_{j=1}^m P_{\sigma | G}(\sigma =\sigma^{(j)})\\
\end{align*}
If we can show that 
...
\begin{align}
E \big[  \exp\big(\beta (A^r_i-A^0_i + A^r_j - A^0_j) \big) ~\big|~ G\in\cG_1 \big] &= (1+o(1))n^{2g(\beta) - 2} \\
E \big[  \exp\big(\beta (A^r_i-A^0_i + A^r_j - A^0_j) \big) \big] &= (1+o(1))n^{2g(\beta) - 2}
\end{align}
\end{lemma}
\begin{proof}
First we have
...
\sum_{t_1=-n/k}^{-1} P(A'^r_i - A^0_i = t_1 \log n)E[\exp(\beta t_1 \log n)] \\
& \cdot
...
Therefore,
$$
E \big[  \exp\big(\beta (A^r_i - A^0_i + A^r_j - A^0_j ) \big) \big] = (1+o(1))\sum_{t=-2\frac{n}{k}}^{-2} E[\exp(\beta t \log n)]
P(A^r_i - A^0_i + A^r_j - A^0_j = t\log n) 
$$
