...
\begin{theorem}
	...
	Then $\frac{T}{(\log n)^3}$ converges to $\frac{1}{k^2}(\frac{a^3}{6} + \frac{k-1}{2}ab^2)$ in probability as $n \to \infty$.
\end{theorem}
\begin{proof}
	We split $T_2$ into two parts, the first is the number of triangles within community $i$, $T_i$. There are $k$ terms of $T_i$.
	The second is the number of triangles which have one node in community $i$ and one edge in community $j$, $T_{ij}$. There are $k(k-1)$ terms of $T_{ij}$.
	We only need to show that $\frac{T_i}{\log ^3 n} \to \frac{(a/k)^3}{6}$ and $\frac{T_{ij}}{\log^3 n} \to \frac{1}{2}(a/k)(b/k)^2$.
	...
	$\Var[T_{ij}] \sim \frac{ab^2}{2k^3} \log^3 n$. Since 
\end{proof}
