...
\end{align}
Using Lemma \ref{lem:Cabd}, we can get $\min_{u \perp g, ||u||_2=1} u^T C u = \frac{1+\kappa}{2}(a-b)\log n := t$
...
& \leq n\cdot \exp \left(
\frac{(a-b)^2(\kappa+1)^2\log n}{8(\kappa+1)^2\log n (a(1-\A) + b(1-\B)) + \frac{2a(1+\kappa)(a-b)}{3}\frac{\log^2 n}{n} }
\right)\\
& \leq n^{-\epsilon}
\end{align*}
where $\epsilon > 0$ by choosing $(a-b)^2>8(a+b)$
