...
we need more finer control by considering the following event $D_r : = \sum_{i=1}^n\exp ( \beta(A^r_i-A^0_i)) > 1$,
$\tau = -\frac{(b-a)\log n}{k}$
\begin{align*}
\sum_{i=1}^n\exp ( \beta(A^r_i-A^0_i)) 
= \sum_{t=-\frac{n}{k}}^{\frac{n}{k}}\sum_{i=1}^n 1[A^r_i - A^0_i]\exp ( \beta  t\log n) 
\end{align*}
we have
\begin{align*}
&\Pr(D_r) = 
...
&+ \Pr(D_r | \tau < A_i^r - A_i^0  < 0)
\Pr( \tau < A_i^r - A_i^0  < 0 ) \\
& \leq \Pr(D_r| A_i^r - A_i^0 \geq 0)
+ \Pr(D_r | A_i^r - A_i^0  < \tau) 
+ \Pr(D_r | \tau < A_i^r - A_i^0  < 0)
\end{align*}
\begin{align*}
&\Pr(D_r | A_i^r - A_i^0  < \tau) \leq \Pr(\sum_{t=-\frac{n}{k}}^{\tau}\sum_{i=1}^n 1[A^r_i - A^0_i]\exp ( \beta  t\log n) > 1) \\
& \leq E[(\sum_{t=-\frac{n}{k}}^{\tau}\sum_{i=1}^n 1[A^r_i - A^0_i]\exp ( \beta  t\log n) ] \\
& \leq n^{1 + \beta(b-a)/k}
\end{align*}
...
$ \Pr(D_r| A_i^r - A_i^0 \geq 0) \leq $
\begin{align*}
\Pr(\sum_{i=1}^n\sum_{r=1}^{k-1}\exp ( \beta(A^r_i-A^0_i)) >  1 ) \\
\end{align*}

where the quantity $t\log(n)$ ranges over all integer values from $-n/k$ to $n/k$ in the summation.
Define $D_r(G,t):=|\{i\in[n]:A^r_i-A^0_i= t\log(n)\}|=\sum_{i=1}^n \mathbbm{1}[A^r_i-A^0_i=t \log(n)]$.
Therefore,
