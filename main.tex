...
That is to say
$$
E_G[\log \frac{P(\sigma = X)}{P(\sigma = \mathbf{1}_n)}] > 0
$$
...
From \eqref{eq:diff_by_one} we can see that the mean value decreases when $\beta > \beta^*$, we can show that $\frac{P_{\sigma|G}(\dist(\sigma, X)=1)}{P_{\sigma|G}(\sigma=X)}$ decreases
in a similar behavior with probability 1 as $n\to \infty$. The analysis of $\frac{P_{\SIBM} ( \dist(\sigma, X) = r )}{P_{\SIBM}(\sigma= X)}$ for $1\le r<kn/\log^{1/3}(n)$ is
similar but with some notation difference.
