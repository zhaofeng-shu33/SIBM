...
The deduction in the last section can be enhanced in the following form:
\begin{align}
...
\end{align}
where $C$ is a positive constant in \eqref{eq:1x_e}. We claim that \eqref{eq:1x_e} holds when $\alpha < b \beta $.
...
\leq (k-1)k^n \exp(-Cn) = o(2)$ for $C> \log k$, we conclude that
...
We notice that when $\beta$ is large, $g(\beta) > 0$. Therefore, Equation \eqref{eq:betastar_x} is not satisfactory, and
