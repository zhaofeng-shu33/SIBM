...
\sum_{j=2}^{n+1} (V_{ij} + V_{ji})& = 0,~\forall i\in\{1,\ldots,n+1\}. \label{eq:sdp}
	...
	It can be verified that $a+\sum_{i=1}^{n} a_i = 2h^Tg +\frac{1}{2} g^T B g$, and hence condition (b) holds.
	...
	\begin{align}\label{eq:Xi_n_pre}
		\Xi_n =&\diag(hg^T + \frac{1}{2}Bgg^T -(d^Tg + b)\mathbbm{1}_ng^T)  \nonumber\\
		& -\frac{1}{2}B + \mathbbm{1}_n d^T + d \mathbbm{1}_n^T,
	\end{align}
	...
	\begin{align}\label{eq:Xi_n_pre_1}
		\Xi_n =&\diag(hg^T + \frac{\kappa+1}{2}Agg^T -(d^Tg + b)\mathbbm{1}_ng^T)  \nonumber\\
		& -\frac{\kappa+1}{2}A + \frac{\kappa}{2}J_n + \mathbbm{1}_n d^T + d \mathbbm{1}_n^T,
	\end{align}
	...
	\begin{align*}
		...
		=& (g^{\perp})^T \diag( -(d^T g+b) \mathbbm{1}_ng^T+hg^T +\frac{\kappa+1}{2} Agg^T) g^{\perp} \\
		&+ \frac{\kappa+1}{2}p 
		+ \frac{1}{2}\left(\kappa -\frac{\kappa+1}{2}(p+q)\right)(g^{\perp})^TJ_n g^{\perp}\\
		&+ 2(g^{\perp} \mathbbm{1}_n)(d^Tg^{\perp})
		-\frac{\kappa+1}{2}(g^{\perp})^T(A-\mathbb{E}[A])g^{\perp},
	\end{align*}
	...
	and
	\textcolor{red}{$\frac{1}{2}(\kappa-\frac{\kappa+1}{2}(p+q))+2\lambda_1=0$}.
	...
	\begin{align}\label{eq:lastterm}
		(g^{\perp})^T \Xi_n g^{\perp} \geq \min_i\{h_i g_i+ \frac{\kappa+1}{2}g_i (Ag)_i \} - c \sqrt{\log n}
	\end{align}
