...
%	\end{proof}
	...
	\begin{align}
		\max_{v}\, & h^T v + \frac{1}{2}v^T B v \notag \\
		s.t.\, & \mathbbm{1}_n^T v = 0 \text{ and } v_i \in \{\pm 1 \} \label{eq:matrix_mle}
	\end{align}
	where $h$ is an n-dimensional vector with entry $h_i = \frac{1}{\log \frac{p(1-q)}{q(1-p)}}\sum_{j=1}^m \log \frac{P_0(x^i_{j})}{P_1(x^i_{j})}$ for $i\in\{1,\ldots,n\}$ and
	for $\kappa>0$ the $n\times n $ matrix $B$ is defined as
	\begin{equation}
		B_{ij} = \begin{cases}
...
\sum_{j=2}^{n+1} (V_{ij} + V_{ji})& = 0,~\forall i\in\{1,\ldots,n+1\}. \label{eq:sdp}
	...
	\begin{theorem}\label{thm:sdp}
		If $I_+ > 1$ and $\kappa=\frac{a-b}{\log(a/b)}\frac{\log n}{n}$,
		then with high probability, the optimal solution
	...
	\begin{align}
		...
		s.t. &\, \diag\{a, a_1, \dots, a_{n}\} + \Xi - \widetilde{B} \succeq 0, 
	\end{align}
	...
	\begin{enumerate}
		...
		\item[(b)] There exists a feasible solution $(a, a_1, \dots, a_{n},$ $\lambda_1,\ldots,\lambda_{n+1})$ to \eqref{eq:dual},
		such that $a+\sum_{i=1}^{n} a_i=Tr((1,g)(1,g)^T\widetilde{B})$.
		\item[(c)] $(\diag\{a, a_1, \dots, a_{n}\} + \Xi - \widetilde{B})(1,g)=0$.
		\item[(d)] The second smallest eigenvalue of $\diag\{a,a_1, \dots, a_{n}\} + \Xi - \widetilde{B}$ is greater than $0$. 
	\end{enumerate}
	Condition (a) holds by definition. It suffices to choose
	$(a, a_1, \dots, a_{n},\lambda_1,\ldots,\lambda_{n+1})$ that satisfies conditions (b), (c) and (d).
    From condition (c) we have that
	\begin{align}
		a &= h^T g \nonumber\\
		a_{i} & = (h_i - d^Tg - b )g_i + \diag\{Bgg^T\}_i, i = 1, \dots, n
	\end{align}

	
	It can be verified that $a+\sum_{i=1}^{n} a_i = 2h^Tg + g^T B g$,
	and hence condition (b) holds.
	
	
	Hence it suffices to prove that
	\begin{align}
		&\diag(\{a, a_1, \dots, a_{n}\}) + \Xi - \widetilde{B} \nonumber\\
		=& \begin{pmatrix} h^T g & -h^T + b\mathbbm{1}_n^T \\
			-h+b\mathbbm{1}_n & \Xi_n \end{pmatrix}
		\succeq 0, 
	\end{align}
	where
	\begin{align}\label{eq:Xi_n_pre}
		\Xi_n =&\diag(hg^T + Bgg^T -(d^Tg + b)\mathbbm{1}_ng^T)  \nonumber\\
		& -B + \mathbbm{1}_n d^T + d \mathbbm{1}_n^T,
	\end{align}
	We introduce the following notation: $\mathbbm{1}_n$ is the all 1's vector, $J_n$ is the all 1's matrix, and $I_n$ is the identity matrix.

	Using the relationship, $B=(1+\kappa) A -\kappa(J_n-I_n)$,
	where $A$ is the adjacency matrix of the graph, we obtain
	\begin{align}\label{eq:Xi_n_pre_1}
		\Xi_n =&\diag(hg^T + (\kappa+1)Agg^T -(d^Tg + b)\mathbbm{1}_ng^T)  \nonumber\\
		& -(\kappa+1)A + \kappa J_n + \mathbbm{1}_n d^T + d \mathbbm{1}_n^T,
	\end{align}
	...
	Using $\tilde{h}^T g = 2(\tilde{h}^T\hat{g}_1)^2 
	+ 2(\tilde{h}^T\hat{g}_2)^2 - [\tilde{h}^T(\hat{g}_1 + \hat{g}_2)]^2$
	, we have 
	\begin{align*} 
		...
		&=2\frac{ \sum_{i<j,i,j\in S_1} (\tilde{h}_i - \tilde{h}_j)^2 + \sum_{i<j,i,j\in S_2} (\tilde{h}_i - \tilde{h}_j)^2 }{n^2}+ \frac{(\tilde{h}^T\mathbbm{1}_n)^2}{n^2}\\
		& = I_1 + I_2 + \frac{(\tilde{h}^T\mathbbm{1}_n)^2}{n^2}.
	\end{align*}
	...
	\textcolor{red}{$\tilde{h}^T\mathbbm{1}_n = h^T \mathbbm{1}_n -nb = 0$}.

	Denote $\mathbb{E}[h_i]=m D_j, \Var[h_i]=m \bar{D}_j, \mathbb{E}[h_i^2]=\Var[h_i]+\mathbb{E}^2[h_i]=m \bar{D}_j+m^2 D^2_j, \Var[h_i^2] \leq m^4 \bar{C}_j$ for $i\in S_j$ and $j\in\{1,2\}$. Then
	...
	For the last term $(g^{\perp})^T \Xi_n g^{\perp}$, by \eqref{eq:Xi_n_pre_1} and the fact that $(g^{\perp})^Tg=0$, we have that
	\begin{align*}
		&(g^{\perp})^T \Xi_n g^{\perp} \\
		=& (g^{\perp})^T \diag( -(d^T g+b) \mathbbm{1}_ng^T+hg^T +(\kappa+1) Agg^T) g^{\perp} \\
		&+ (\kappa+1)p 
		+ \left(\kappa -\frac{\kappa+1}{2}(p+q)\right)(g^{\perp})^TJ_n g^{\perp}\\
		&+ 2(g^{\perp} \mathbbm{1}_n)(d^Tg^{\perp})
		-(\kappa+1)(g^{\perp})^T(A-\mathbb{E}[A])g^{\perp},
	\end{align*}
	...
	To make both the coefficients of
	$\mathbbm{1}_ng^T$ and $(g^{\perp})^TJ_n g^{\perp}$ vanish,
	we choose $d=\lambda_1 \mathbbm{1}_n + \lambda_2 g$.
	Then \textcolor{red}{$n\lambda_2+b=0$}
	and
	\textcolor{red}{$\kappa-\frac{\kappa+1}{2}(p+q)+2\lambda_1=0$}.
	...
	\begin{align}\label{eq:lastterm}
		(g^{\perp})^T \Xi_n g^{\perp} \geq \min_i\{h_i g_i+ (\kappa+1)g_i (Ag)_i \} - c \sqrt{\log n}
	\end{align}
	% The error probability is then bounded by
	% \begin{align*}
	% P(\Xi_{ii} \leq c\sqrt{\log n}, \forall 1\leq i \leq n)
	% \end{align*}
	Below we show that with probability $1-n^{-I(\kappa)+o(1)}$,
	the following inequality holds
	\begin{align}\label{eq:nodei}
		 \frac{1}{\kappa+1}h_i  g_i  + g_i (Ag)_i  - c \sqrt{\log n}\ge 0
	\end{align}
	for $i\in\{1,\ldots,n\}$.
	where $I(\kappa)$ is defined for $\kappa>0$ in a similar way to $I_1$ 
	\begin{align}
		I(\kappa)=\min_{P_{\widetilde{X}_1}} \gamma D(P_{\widetilde{X}_1}|| P_0)+ \frac{1}{2} g(a,b, 2\epsilon),\text{ where}\nonumber\\
		\epsilon = \frac{\gamma}{\kappa+1} \frac{D(P_{\widetilde{X}_1} || P_1) - D(P_{\widetilde{X}_1} || P_0) }{\log a /b}\label{eq:Ikappa}
	\end{align}
	Since $g(a,b,\epsilon)$ is an increasing function about $\epsilon$,
	we find that $I(\kappa)$ is a decreasing function about $\kappa$
	and the smallest error is achieved when $\kappa \to 0^+$.
	For ML estimator, $\kappa = -\log\frac{1-p}{1-q} / \log\frac{p}{q} \sim \frac{a-b}{\log a/b}\frac{\log n}{n}$,
	which satisfied $\kappa\to 0$ as $n\to 0$.
	This coincidence demonstrates the optimality
	of ML estimator and we can use it to construct the SDP algorithm to achieve the optimal
	threshold.

	To obtain the error probability
	$n^{-I(\kappa)+o(1)}$, which is the probability
	that \eqref{eq:nodei} is violated.
	For $g_i=1$,
	...
	follow binomial distributions. The term $h_i$ can be written as
	$h_i=m \frac{D(P_{\widetilde{X}_i} || P_1) - D(P_{\widetilde{X}_i} || P_0) }{\log a /b}$, where $P_{\widetilde{X}_i}$ is the empirical distribution of samples $\{X^i_j\}^m_{j=1}$ at node $i$. 
	We then apply the following lemma, which is a slight generalization of Lemma 8 in \cite{abbe2015exact} and can be proved using Chernoff bound.
	...
	$n^{-I(\kappa) + o(1)}$ for any $i$. By the union bound, the probability that there exists an $i$ that violates \eqref{eq:nodei} is at most $n^{-I(\kappa)+1 + o(1)}$, which fades to zero if we choose $\kappa$ sufficient small such that  $I_1=I(0)>I(\kappa)>1$.
