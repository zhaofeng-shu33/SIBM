...
\sum_{j=2}^{n+1} (V_{ij} + V_{ji})& = 0,~\forall i\in\{1,\ldots,n+1\}. \label{eq:sdp}
	...
	Using the relationship, $B=2A+I_n-J_n$,
	where $A$ is the adjacency matrix of the graph, we obtain
	\begin{align}\label{eq:Xi_n_pre_1}
		\Xi_n =&\diag(hg^T + Agg^T -(d^Tg + b)\mathbbm{1}_ng^T)  \nonumber\\
		& -A + \frac{1}{2}J_n + \mathbbm{1}_n d^T + d \mathbbm{1}_n^T,
	\end{align}
	...
	\begin{align*}
		...
		=& (g^{\perp})^T \diag( -(d^T g+b) \mathbbm{1}_ng^T+hg^T + Agg^T) g^{\perp} + p \\
		&+ \frac{1}{2}(1-p-q)(g^{\perp})^TJ_n g^{\perp} + 2(g^{\perp} \mathbbm{1}_n)(d^Tg^{\perp})\\
	...
	and
	\textcolor{red}{$\frac{1}{2}(1-p-q)+2\lambda_1=0$}.
	...
	\begin{align}\label{eq:lastterm}
		(g^{\perp})^T \Xi_n g^{\perp} \geq \min_i\{h_i g_i+ g_i (Ag)_i \} - c \sqrt{\log n}
	\end{align}
